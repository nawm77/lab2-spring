...
public interface OfferController {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Заказ успешно создан",
                    content = @Content(mediaType = JSON_TYPE, schema = @Schema(implementation = OfferDTO.class))),
            @ApiResponse(responseCode = "400", description = "Некорректные данные",
                    content = @Content(mediaType = JSON_TYPE, schema = @Schema(implementation = ApiErrorResponse.class))),
            @ApiResponse(responseCode = "409", description = "Заказ уже существует",
                    content = @Content(mediaType = JSON_TYPE, schema = @Schema(implementation = ApiErrorResponse.class)))
    })
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Заказ успешно найден",
                    content = @Content(mediaType = JSON_TYPE, schema = @Schema(implementation = OfferDTO.class))),
    ...
    ResponseEntity<?> getOffer(
            @Parameter(in = ParameterIn.QUERY, description = "ID заказа", schema = @Schema(defaultValue = "-1"))
            @RequestParam(value = "offerId", required = false, defaultValue = "-1") String offerId
    );
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Заказ создан",
                    content = @Content(mediaType = JSON_TYPE, schema = @Schema(implementation = OfferDTO.class))),
            @ApiResponse(responseCode = "202", description = "Заказ изменен",
                    content = @Content(mediaType = JSON_TYPE, schema = @Schema(implementation = OfferDTO.class))),
            @ApiResponse(responseCode = "400", description = "Некорректные данные",
                    content = @Content(mediaType = JSON_TYPE, schema = @Schema(implementation = ApiErrorResponse.class)))
    })
    ...
    ResponseEntity<?> deleteOffer(
            @Parameter(in = ParameterIn.QUERY, description = "ID заказа", schema = @Schema())
            @RequestParam(value = "offerId") String id
    );
}
